        ...
        TokenValue ->
            case verify_token(binary_to_list(TokenValue)) of
                {ok, Username} ->
                    ...
                    user_registry:add_user(Username, Pid),
...
websocket_handle({text, Msg}, State = #{username := Sender}) ->
    try jsx:decode(Msg, [return_maps]) of
        ...
        #{<<"type">> := <<"get_contacts">>, <<"user">> := User} ->
            case contact_storage:get_contacts(User) of 
                {ok, Contacts} -> 
                    {reply, {text, jsx:encode(#{
                        <<"type">> => <<"contacts">>,
                        <<"contact">> => Contacts})}, State};
                _ ->
                    io:format("Error"),
...
terminate(_Reason, _Req, State) ->
    case is_map(State) of
        true -> 
            ...
            io:format("WebSocket connection closed for user ~p~n", [Username]),
